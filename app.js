...
async function sendSubscriptionToServer(subscription) {
  ...
  if (!res.ok) {
    throw new Error('Сервер не принял подписку: ' + res.status);
  }
...
async function subscribeToPush() {
  ...
  try {
    ...
    const subscription = await registration.pushManager.subscribe({
      ...
      applicationServerKey: urlBase64ToUint8Array('BDg2P-0XSuK2usqCC9sgFVRx-nB9FmHX0UI-9g3vy0w2thraqaZ-WbmonQej3mZTL0ZJwUPdG-ql3XkbEnPPeUw'),
    });
    await sendSubscriptionToServer(subscription);
  } catch (err) {
    console.error('Ошибка подписки на push:', err);
    alert('Не удалось подписаться на уведомления. См. консоль (F12).');
  }
}
    ...
    try {
      ...
      if (enableBtn && disableBtn) {
        ...
        if (subscription) {
          ...
          // после перезапуска server.js подписка в браузере есть, на сервере — нет
          try {
            await sendSubscriptionToServer(subscription);
          } catch (err) {
            console.error('Не удалось обновить подписку на сервере:', err);
          }
        }
